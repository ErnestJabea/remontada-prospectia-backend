...
router.get('/active-users', authenticate, authorize('SYSTEM', 'ADMIN'), async (req, res) => {
  try {
...
router.get('/login-history', authenticate, authorize('SYSTEM', 'ADMIN'), async (req, res) => {
  try {
    const [rows] = await pool.query(`
...
router.get('/audit-logs', authenticate, authorize('SYSTEM', 'ADMIN'), async (req, res) => {
  try {
    const [rows] = await pool.query(`
